...
class levelFiveScreen extends StatefulWidget {
  final String UsernamelevelFive;
...
class _LevelFiveScreenState extends State<levelFiveScreen> {
  ...
  void _fetchQuizData() async {
    final body = jsonEncode({
      'name': widget.UsernamelevelFive,
    });

    name = widget.UsernamelevelFive;

    print('Name being sent level five result: ${name}');

    try {
      final response = await http.post(
        Uri.parse('http://192.168.18.3:7020/quiz_leval_Five'),
