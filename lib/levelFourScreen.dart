...
class levelFourScreen extends StatefulWidget {
  final String UsernamelevelFour;

  const levelFourScreen({
    Key? key,
    required this.UsernamelevelFour,
...
class _LevelFourScreenState extends State<levelFourScreen> {
  ...
  void _fetchQuizData() async {
    final body = jsonEncode({
      'name': widget.UsernamelevelFour,
    });

    name = widget.UsernamelevelFour;

    print('Name being sent level four result: ${name}');

    try {
      final response = await http.post(
        Uri.parse('http://192.168.18.3:7020/quiz_leval_Four'),
