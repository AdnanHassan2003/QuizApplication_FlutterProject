...
class levelTwoScreen extends StatefulWidget {
  final String UsernamelevelTwo;

  const levelTwoScreen({
    Key? key,
    required this.UsernamelevelTwo,
...
class _LevelTwoScreenState extends State<levelTwoScreen> {
  ...
  void _fetchQuizData() async {
    final body = jsonEncode({
      'name': widget.UsernamelevelTwo,
    });

    name = widget.UsernamelevelTwo;

    print('Name being sent level two result: ${name}');

    try {
      final response = await http.post(
        Uri.parse('http://192.168.18.3:7020/quiz_leval_Two'),
